...
// ✅ Pre-create temp cache directory with full permissions
const tempCachePath = path.join(os.tmpdir(), 'synk-cache');
try {
  if (!fs.existsSync(tempCachePath)) {
    fs.mkdirSync(tempCachePath, { recursive: true, mode: 0o777 });
  }
  console.log(`✅ Cache directory ensured: ${tempCachePath}`);
  
  // Set environment variables for Electron cache BEFORE electron module loads
  process.env.ELECTRON_CACHE = tempCachePath;
  process.env.CACHE_DIR = tempCachePath;
  
  // GPU crash prevention - disable GPU early via environment
  process.env.DISABLE_GPU_COMPOSITING = '1';
  process.env.ANGLE_DEVICE_TYPE = 'warp';
} catch (err) {
  console.warn(`⚠️ Could not create cache dir: ${err.message}`);
}
...
// Disable GPU and fix cache issues
const electron = require('electron');
const { app } = electron;

// Comprehensive GPU disable
app.commandLine.appendSwitch('disable-gpu');
app.commandLine.appendSwitch('disable-gpu-compositing');
...
const userDataPath = path.join(os.tmpdir(), 'synk-userData');
if (!fs.existsSync(userDataPath)) {
  fs.mkdirSync(userDataPath, { recursive: true, mode: 0o777 });
}
app.commandLine.appendSwitch('user-data-dir', userDataPath);

app.disableHardwareAcceleration();

console.log('✅ GPU disabled via multiple methods');
...
if (app && BrowserWindow && ipcMain) {
  ...
  app.whenReady().then(() => {
    ...
    // ✅ Final GPU disable measures on app ready
    try {
      app.disableHardwareAcceleration();
      app.commandLine.appendSwitch('disable-gpu');
      app.commandLine.appendSwitch('disable-gpu-compositing');
      console.log('✅ GPU fully disabled on app ready');
    } catch (e) {
      console.warn('GPU disable on app ready failed:', e.message);
    }
    ...
    // Handle GPU process crashes silently (don't crash the app)
