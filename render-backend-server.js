  ...
  webhookPaths.forEach((p) => app.post(p, express.raw({ type: '*/*' }), async (req, res) => {
    ...
    try {
      ...
      async function applySubscriptionToUser({ priceId, status, customerId, emailHint, subscription }) {
        ...
        if (!userRow) {
          try {
            await insertUser({ email, password_hash: null, plan: null, billing_period: null, is_trial: false, trial_end: null });
        ...
        // If subscription is canceled/unpaid/expired, clear plan; otherwise set plan for active/trialing
        try {
          // Handle canceled, unpaid, or expired subscriptions - SET PLAN TO NULL and exit
          if (status === 'canceled' || status === 'unpaid' || status === 'incomplete_expired') {
            ...
            console.log(`✅ [Stripe] Plan CLEARED for ${email} due to status: ${status}`);
            return; // Exit after clearing the plan
          }
          ...
          if (status === 'active' || status === 'trialing' || status === 'past_due') {
            if (isTrialSubscription) {
              ...
              console.log(`✅ [Stripe] TRIAL started for ${email}. Plan: ${mapping.plan}, Billing: ${mapping.billing_period}`);
            } else {
              ...
              console.log(`✅ [Stripe] PLAN activated for ${email}. Plan: ${mapping.plan}, Billing: ${mapping.billing_period}`);
            }
          }
        } catch (e) {
      ...
      switch (event.type) {
        case 'checkout.session.completed': {
          ...
          if (session.subscription) {
            ...
            // Pass full subscription for trial detection
        ...
        case 'customer.subscription.deleted': {
          ...
          const item = sub.items && sub.items.data && sub.items.data[0];
          const priceId = item && item.price && item.price.id;
          // Explicitly mark as canceled to clear plan
          await applySubscriptionToUser({ 
...
app.get('/me', authMiddleware, async (req, res) => {
  try {
    ...
    if (u.is_trial && u.trial_end && new Date() < new Date(u.trial_end)) {
      isTrialActive = true;
      trialTimeRemaining = Math.max(0, new Date(u.trial_end) - new Date());
    } else if (u.is_trial && u.trial_end && new Date() >= new Date(u.trial_end)) {
      // Trial expired - update user to remove trial status
      await updateUser(email, { 
        plan: null, 
        billing_period: null, 
        is_trial: false,
